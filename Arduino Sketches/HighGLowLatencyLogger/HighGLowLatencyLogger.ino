...
const uint16_t Xzero = 32768; //LSB per milliG
const int XGain = 1; //LSB per milliG
const uint16_t Yzero = 32768; //LSB per milliG
const int YGain = 1; //LSB per milliG
...
const uint8_t Accel_CS_PIN = 21;

const uint8_t recordSwitchPin = 23;

//
// Digital pin to indicate an error, set to -1 if not used.
// The led blinks for fatal errors. The led goes on solid for SD write
// overrun errors and logging continues.
const int8_t ERROR_LED_PIN = 17;
...
void acquireData(data_t* data) {
  if(!buffer0->isEmpty() && !buffer1->isEmpty() ) {
    data->time = micros();
    data->adc[0] = uint16_t(buffer0->read());
    data->adc[1] = uint16_t(buffer1->read());
    
  }
//Use for live display at slow rates only
//  Serial.print(data->adc[0]);
//  Serial.print("\t");
...
const uint32_t FILE_BLOCK_COUNT = 20480;
...
uint32_t const ERASE_SIZE = 26214400L;
void logData() {
  uint32_t bgnBlock, endBlock;
